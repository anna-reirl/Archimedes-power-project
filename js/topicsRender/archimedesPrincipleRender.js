...
const archimedesPrincipleDemo = {
    ...
    passFrameValue: function (volume, buyonantForce, gravity, depth) {
    ...
    */
    ...
    stoppedDrawDrag: function (dropElem) {
        ...
        if (dropElem.elem.x > (xEndDynam - this.constants.dropAreaSize) &&
            ...
            let weight = applicationRendering.topicVariables.weight; // m
            let bodyDensity = applicationRendering.topicVariables.bodyDensity; // p
            let liquidDensity = applicationRendering.topicVariables.liquidDensity; // p жид
            let volume = +(weight / bodyDensity).toFixed(4); // V

            const widthRatio = +applicationRendering.topicVariables.bodyWidth;
            const heightRatio = +applicationRendering.topicVariables.bodyHeight;
            const thicknessRatio = +applicationRendering.topicVariables.bodyThickness;
            const onePart = +Math.cbrt(volume / (widthRatio * heightRatio * thicknessRatio)).toFixed(2);
            const height = heightRatio * onePart;
            const area = widthRatio * thicknessRatio * (onePart ** 2); // площадь основания


            let buyonantForce = +(liquidDensity * 9.81 * volume).toFixed(4); // F архимеда



            let gravityForce = this.getForce();
            const depthVolume = +(weight / liquidDensity).toFixed(4); // объем погружения
            let depthHeight; // высота погружения
            if(depthVolume <= volume){
                if(depthVolume < 0.01) depthHeight = 0;
                else depthHeight = +(depthVolume / area).toFixed(2);
            } else {
                depthHeight = -1;
            }
            ...
            this.dynamometers.springDynamometer.setValue(gravityForce).then(() => {
                ...
                this.animatePosition(height * 100, depthHeight * 100).then(() => {
            ...
            this.passFrameValue(volume, buyonantForce, +this.getForce().toFixed(4), depthHeight);
    ...
    animatePosition: async function (height, deptHeight) {
        ...
        if(deptHeight >= 0) liquidLevelY -= height - deptHeight;
        let yEndDynam = this.getDropPosition().y;
    ...
    */
    receivedMessage: async function () {
        ...
        if( this.dynamometers != undefined)
        {
            ...
            const weight = applicationRendering.topicVariables.weight; // m
            const bodyDensity = applicationRendering.topicVariables.bodyDensity; // p
            const volume = +(weight / bodyDensity).toFixed(4); // V
            const widthRatio = +applicationRendering.topicVariables.bodyWidth;
            const heightRatio = +applicationRendering.topicVariables.bodyHeight;
            const thicknessRatio = +applicationRendering.topicVariables.bodyThickness;
            const onePart = +Math.cbrt(volume / (widthRatio * heightRatio * thicknessRatio)).toFixed(2);

            this.getBrick().width = onePart * widthRatio * sizeRatio;
            this.getBrick().height = onePart * heightRatio * sizeRatio;
